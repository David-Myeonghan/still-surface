...
**모바일**: 왼쪽 화면 드래그 = 이동(가상 조이스틱, 세게 밀면 달리기) · 오른쪽 드래그 = 시선 · `JUMP` 버튼 = 점프 · 비콘 근처에서 `SCAN` 버튼 홀드 = 스캔 · `DASH` 버튼 = 대시.

`E` (또는 모바일 `DASH` 버튼) = 모트를 태워 대시. 7개 스캔 후 결과 화면에서 기록 확인 + "다시(새 행성)"·"오늘의 행성(데일리)". `?seed=숫자`로 같은 행성 공유.
결과 화면의 **🖼 결과 카드**로 기록 이미지를 저장/공유(모바일은 Web Share). 링크엔 OG 미리보기 카드가 붙는다.
